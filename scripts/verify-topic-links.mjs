...
assert.deepEqual(topicsForVideo({ ...base, name: 'Untitled', genre: 'Documentary' }), []);
...
try {
  ...
  await page.getByText('3 of 4 public titles linked', { exact: false }).waitFor();
  await page.screenshot({ path: 'screenshots/topic-stats-fixture.png' });
  await page.getByRole('button', { name: '#gaming · 3 · ↔', exact: true }).click();
  ...
  await page.getByRole('button', { name: '#learning · 1 shared', exact: true }).click();
