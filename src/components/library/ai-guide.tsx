...
export function AiGuide() {
  ...
  return <section className="w-full max-w-none rounded-xl bg-surface p-5 shadow-border sm:p-6 xl:p-8"><p className="flex items-center gap-2 text-xs font-medium tracking-[0.16em] text-accent uppercase"><Sparkles className="size-4" />Library guide</p><h1 className="mt-3 font-display text-4xl text-fg sm:text-5xl">Useful answers from local signals.</h1><p className="mt-3 max-w-xl text-sm text-muted">This guide only uses saved favorites, likes, tags, viewing history, and the most recent Twitch refresh. It does not send your catalog or files to a cloud model.</p><div className="mt-6 rounded-lg bg-elevated p-5 shadow-border"><div className="flex items-center justify-between gap-3"><div><p className="font-display text-2xl text-fg">Your current signals</p><p className="mt-1 text-xs text-muted">{history.length} history signals · {Object.keys(favorites).length} favorites · {Object.values(tags).flat().length} tags</p></div><WandSparkles className="size-5 text-accent"/></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{localPicks.map((video) => <button key={video.id} type="button" onClick={() => openPreview(video.id)} className="rounded-md bg-bg/45 px-3 py-3 text-left shadow-border hover:bg-bg"><p className="truncate text-sm font-medium text-fg">{video.name}</p><p className="mt-1 truncate text-xs text-muted">{video.remote?.channelName ?? video.genre ?? "Library pick"} · {(tags[video.id] ?? []).slice(0, 3).join(", ") || "Fresh discovery"}</p></button>)}</div></div><div className="mt-5 rounded-lg border border-border bg-elevated/60 p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-medium text-fg">Twitch live check</p><p className="mt-1 text-xs text-muted">{liveNow.length ? `${liveNow.length} followed channel${liveNow.length === 1 ? "" : "s"} live in the latest refresh.` : "No followed channels are live in the latest refresh."}</p></div><span className="rounded-full bg-accent/15 px-2 py-1 text-xs font-medium text-accent">{liveNow.length} live</span></div></div><div className="mt-6 flex flex-col gap-2 sm:flex-row"><Input value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void ask(); }} aria-label="Library guide question"/><Button disabled={busy || !prompt.trim()} onClick={() => void ask()}><Bot className="size-4" />{busy ? "Checking library…" : "Ask guide"}</Button></div>{answer && <div className="mt-5 rounded-lg bg-elevated p-5 text-sm leading-6 text-fg shadow-border whitespace-pre-wrap">{answer}</div>}<div className="mt-5 flex flex-wrap gap-2">{["Who is live on Twitch?", "Something funny", "A tech video", "What fits my favorites?", "Show a surprise based on my tags"].map((suggestion) => <Button key={suggestion} size="sm" variant="secondary" onClick={() => setPrompt(suggestion)}>{suggestion}</Button>)}</div></section>;
}
