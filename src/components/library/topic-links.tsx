...
export function TopicLinks({ explorer = false }: { explorer?: boolean }) {
  ...
  const index = useMemo(() => {
    ...
    for (const row of rows) {
      ...
      for (const topic of row.topics) {
        const entry = counts.get(topic) ?? { count: 0, providers: new Set<string>() };
        entry.count++; entry.providers.add(row.provider); counts.set(topic, entry);
      }
    }
    return { rows, saved, linked, counts: [...counts].sort((a, b) => b[1].count - a[1].count), gaps: [...sources].map(([id, s]) => ({ id, ...s })).sort((a, b) => (b.total - b.linked) - (a.total - a.linked)).slice(0, 8) };
  }, [videos, tags, folders, unavailable, hideDemo]);
  ...
  const related = useMemo(() => {
    ...
    if (selected) for (const row of matching) for (const topic of row.topics) if (topic !== selected) counts.set(topic, (counts.get(topic) ?? 0) + 1);
    return [...counts].sort((a, b) => b[1] - a[1]).slice(0, 8);
  }, [matching, selected]);
  const choose = (topic: string) => { setLimit(48); setProvider('all'); openTopic(topic); };
  const exportLinks = () => {
    const rows = [['topic', 'public_titles', 'providers', 'saved_tag_titles', 'inferred_only_titles'], ...index.counts.map(([topic, data]) => {
      const saved = index.rows.filter((r) => topicEvidence(r.video, tags[r.video.id]).some((link) => link.topic === topic && link.saved)).length;
      return [topic, data.count, [...data.providers].join(' + '), saved, data.count - saved];
    })];
  ...
  return <section className="mt-6 rounded-lg bg-elevated p-5 shadow-border">
    ...
    <div className="mt-4 flex flex-wrap gap-2">{index.counts.map(([topic, data]) => <Button key={topic} size="sm" variant={selected === topic ? 'default' : 'secondary'} onClick={() => choose(topic)} title={[...data.providers].join(' + ')}>#{topic} · {data.count.toLocaleString()}{data.providers.size > 1 ? ' · ↔' : ''}</Button>)}</div>
    ...
    {explorer ? <>
      ...
      {related.length > 0 && <div className="mt-4"><p className="text-xs text-muted">Related through titles in this view</p><div className="mt-2 flex flex-wrap gap-2">{related.map(([topic, count]) => <Button key={topic} size="sm" variant="secondary" onClick={() => choose(topic)}>#{topic} · {count} shared</Button>)}</div></div>}
